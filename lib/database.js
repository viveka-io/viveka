...
schemas.labels = new mongoose.Schema({
...
models.Labels       = mongoose.model('Labels', schemas.labels);
...
async function getBaselineFingerPrint(id) {
    try {
        return await models.FingerPrint
            .findOne({ testId: id, approved: true }, { screenshot: 1, finished: 1 })
            .sort('-finished')
            .exec();
...
async function getLatestFingerPrint(id) {
    try {
        return await models.FingerPrint
            .findOne({ testId: id }, { screenshot: 1 })
            .sort('-created')
            .exec();
...
async function createFingerPrint(data) {
    try {
        incrementUnseenFingerprintCount(data.testId);
...
async function incrementUnseenFingerprintCount(testId) {
    try {
        return await models.Test
            .update({ _id: testId }, {
                $inc: { unseenFingerprintCount: 1 }
            });
    } catch (err) {
        throw new VError(err, 'Failed to increment unseen fingerprint count for test id: ' + testId);
    }
}

async function decrementUnseenFingerprintCount(testId) {
    try {
        return await models.Test
            .update({ _id: testId }, {
                $inc: { unseenFingerprintCount: -1 }
        ...
        throw new VError(err, 'Failed to decrement unseen fingerprint count for test id: ' + testId);
    }
}

async function approveFingerPrint(id) {
    try {
        const fingerprint = await models.FingerPrint
            .findOne({ _id: id }, { testId: 1, approved: 1 })
            .exec();
        
        if (typeof fingerprint.approved === 'undefined') {
            decrementUnseenFingerprintCount(fingerprint.testId);
        }
...
async function createLabel(data) {
    try {
        log.info('Saving label to the DB');

        return await models.Labels
            .create({
                label: data.label,
                type: data.type
            });
        ...
        throw new VError(err, 'Failed to save label');
    }
}
