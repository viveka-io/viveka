...
const log           = require('bunyan').createLogger({name: 'fingerprint-generator'});
...
const waitForJQuery = 'return window.jQuery !== undefined';

async function createFingerPrint(config, mode) {
    try {
        const driver = driverBuilder.build(config);
        const url = mode ? appendQuery(config.url, {viveka_mode: mode}) : config.url;
