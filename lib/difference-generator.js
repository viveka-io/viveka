...
function compare(a, imgA, b, imgB) {
    ...
    if (a && b) {
        ...
        console.log(nameMatching, offsetMatching, styleMatching, isMatchingSize(a, b));
...
function cleanUpDiff(diff, imgA, imgB) {
    ...
    if (config.cleanup) {
        result = result.filter(function(diffItem, j) {
            ...
            if (diffItem.a && (diffItem.deleteA || pixelsChangedOnly)) {
                delete result[j].a;
                delete result[j].deleteA;
                delete result[j].aHasCopy;
            }

            if (diffItem.b && (diffItem.deleteB || pixelsChangedOnly)) {
                delete result[j].b;
                delete result[j].deleteB;
                delete result[j].bHasCopy;
            }

            return result[j].a || result[j].b;
        });
    }
