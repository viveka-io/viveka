...
async function createFingerPrint({id: testId}) {
    ...
    const compressedDomTree = await new Promise((resolve, reject) => {
        zlib.deflate(JSON.stringify(domTree), function (err, buffer) {
            if (err) {
                reject(err);
            }
...
// UNCOMPRESS EXAMPLE
/*
    const buffer = new Buffer(compressedDomTree, 'base64');
...
*/
    await new Promise((resolve, reject) => {
        fs.writeFile('tmp/' + fingerprint._id + '.json', compressedDomTree, null, err => {
            if (err) {
                reject(err);
            }
