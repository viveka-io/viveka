...
async function createFingerPrint({id: testId}) {
    ...
    let fingerprint             = await db.createFingerPrint({ testId });

    log.info(`Generating fingerprint: ${fingerprint._id}`);

    let fingerPrintData         = await generator.createFingerPrint(test.config, baseline ? 'latest' : 'baseline');
    const screenshot            = fingerPrintData.screenshot;
    fingerPrintData.screenshot  = `/screenshot/${testId}/${fingerprint.id}.png`;
    const filename              = 'tmp' + fingerPrintData.screenshot;


    // this should be guarded against empty screenshot
    ...
    try {
        await db.updateFingerPrint(fingerprint.id, fingerPrintData);
