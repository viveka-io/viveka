...
async function createFingerPrint({id: testId}) {
    ...
    fingerprint.screenshot = `/fingerprints/${fingerprint.id}.png`;
    fingerprint.state = 'DONE';
    fingerprint.finished = new Date();

    const filename = 'tmp/public' + fingerprint.screenshot;
...
app.use('/', express.static(appRoot + '/tmp/public/admin'));
app.use('/fingerprints', express.static(appRoot + '/tmp/public/fingerprints'));
app.use('/dev-section', express.static(appRoot + '/tmp/public/dev-section'));
